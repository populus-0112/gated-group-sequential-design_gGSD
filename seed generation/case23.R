...
threshold_futility=rbind(c(0.85,0.83),c(0.9,0.9),c(0.85,0.83))   # subgroup, full group
...
alpha_s_ht=c(0,0.0102,0,0.0148) # when 2 hypotheses of full group are not tested, their alpha are set to be 0
# initialize alpha for 4 hypotheses
alpha_f_ht=c(0.01012,0,0.01488,0) # when 2 hypotheses of subgroup are not tested, their alpha are set to be 0
...
n_futility=405    #n_PFS_2*tau[j]  # No. of PFS events for futility
...
for (j in 1:length(tau)){  # lambda_OS and futility threshold are paired with lambda_PFS
  for (i in 1:nrow(lambda_PFS)){
    ...
    while (t<=iter){  
      ...
      x <- cutDataAtCount(PFS,round(n_futility)) # cut by No. of events
...
if (!is.null(seed_fail)){
  write(t(seed_fail),paste0('case23_seed_fail', ".csv"),ncol=1,sep=',')
}
